...
void *videoPlay(void *args){
    ...
    int numBytes = av_image_get_buffer_size(AV_PIX_FMT_RGBA, ffmpegVideo->codec->width, ffmpegVideo->codec->height,
                                            1);
    //缓存区
...
//        }
        ...
        if (ret < 0 && ret != AVERROR(EAGAIN) && ret != AVERROR_EOF)
            continue;
        if(avcodec_receive_frame(ffmpegVideo->codec, frame) != 0)continue;

//        while(avcodec_receive_frame(ffmpegVideo->codec, frame) == 0){
...
void FFmpegVideo::setFFmepegMusic(FFmpegVideoMusic *ffmpegMusic) {
